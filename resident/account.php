<html>
<head>
<title>Resident Account | Micro Loft</title>
<link  type="text/css" rel="stylesheet" href="/styles/microstyle.css">
</head>

<body>

<?php
	include '/home/frankencluster/public_html/group01/portal_navbar.php';
?>

<div>
	<h1 class="header">Resident Account</h1>
<center>
<form name="resident_account_form" action="">	
	<table class="hoverTable">
		<tr>
			<th class="residentaccount_form">Unit</th>
			<th class="residentaccount_form">Name</th>
			<th class="residentaccount_form">Date Created</th>
		</tr>
		<tr>
			<td class="residentaccount_form">info</td>
			<td class="residentaccount_form">info</td>
			<td class="residentaccount_form">info</td>
		</tr>
	</table>
</form>
</center>
</div>

</body>

</html>